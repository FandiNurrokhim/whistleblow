...
const TYPE_LABEL = {
    manager_to_staff: "Manajer → Staff",
...
const AssessmentTable = () => {
    ...
    const fetchData = () => {
        setIsLoading(true);
        axios
            .get(route("assessment.data"), { params: { page: currentPage, search } })
            .then(({ data: res }) => {
                setData(res.data);
    ...
    const openEdit = (row) => {
        axios.get(route("assessment.edit", row.id)).then(({ data: res }) => {
    ...
    const handleDelete = () => {
        ...
        axios
            .delete(route("assessment.destroy", selectedId))
            .then(() => {
                Swal.fire({ icon: "success", title: "Berhasil", text: "Penilaian dihapus.", timer: 1500, showConfirmButton: false });
                setRefetch((p) => !p);
            })
    ...
    const handleBulkDelete = () => {
        setIsProcessing(true);
        axios
            .post(route("assessment.bulk-delete"), { ids: selectedIds })
            .then(() => {
                Swal.fire({ icon: "success", title: "Berhasil", text: `${selectedIds.length} data dihapus.`, timer: 1500, showConfirmButton: false });
                setRefetch((p) => !p);
            })
    ...
    const columns = [
        { header: "Penilai",  accessor: (row) => row.assessor?.name  ?? "-" },
        { header: "Dinilai",  accessor: (row) => row.assessee?.name  ?? "-" },
        { header: "Periode",  accessor: (row) => row.period          ?? "-" },
        { header: "Tipe",     accessor: (row) => TYPE_LABEL[row.type] ?? row.type },
        { header: "Skor",     accessor: (row) => row.final_score != null ? row.final_score : "-", className: "text-center" },
        {
            header: "Status",
            accessor: (row) => (
    ...
    return (
        <Card>
            ...
            <ConfirmDeleteModal
                ...
                description={`Hapus ${selectedIds.length} penilaian terpilih?`}
