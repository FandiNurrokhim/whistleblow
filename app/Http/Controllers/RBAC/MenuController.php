...
class MenuController extends Controller
{
    public function index(Request $request)
    {
        try {
            if ($request->ajax()) {
                $menus = Menu::with(['parent', 'children'])->orderBy('id');

                return DataTables::of($menus)
                    ->addIndexColumn()
                    ->editColumn('type', function ($menu) {
                        if ($menu->type === 'HEADER') {
                            return '<span className="badge bg-primary">' . $menu->type . '</span>';
                        } elseif ($menu->type === 'PARENT') {
                            return '<span className="badge bg-success">' . $menu->type . '</span>';
                        } elseif ($menu->type === 'SUB PARENT') {
                            return '<span className="badge bg-info">' . $menu->type . '</span>';
                        }
                        return $menu->type;
                    })
                    ...
                    ->addColumn('action', function ($menu) {
                        ...
                        $btn = '<a href="' . $editUrl . '" className="btn btn-warning btn-sm btn-edit"
                            data-id="' . $menu->id . '"
                            data-type="' . $menu->type . '"
                        ...
                        $btn .= '<button className="btn btn-danger btn-sm btn-delete" data-id="' . $menu->id . '">
                            Delete
                        </button>';
